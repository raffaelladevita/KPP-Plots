...
public class EPIPLUSmonitor extends AnalysisMonitor {
    ...
    public EPIPLUSmonitor(String name) {
        super(name);
...
//        hi_rec_w.setOptStat("1110"); 
        ...
        H1F hi_rec_mm = new H1F("hi_rec_mm","hi_rec_mm",100, 0.6, ebeam*0.6+0.6);
        ...
        H2F hi_w_mm = new H2F("hi_w_mm", "hi_w_mm", 100, 0.6, ebeam*0.6+0.4, 100, 0.6, ebeam*0.6+0.6);
...
//        hi_dphi.setOptStat("1110");
        ...
        for(int sector=1; sector <= 6; sector++) {
            H1F hi_rec_mm_sec = new H1F("hi_rec_mm_" + sector, "hi_rec_mm_" + sector, 100, 0.6, ebeam*0.6+0.6);
...
//                if(hadronSystem.mass()>1.1 && hadronSystem.mass()<1.4) {
                    ...
                    this.getDataGroup().getItem(3).getH1F("hi_rec_mm_" + secEl).fill(missingParticle.mass());
                    this.getDataGroup().getItem(4).getH1F("hi_dphi_" + secEl).fill(Math.abs(Math.toDegrees(recPi.phi()-recEl.phi())));
//                }
            }
